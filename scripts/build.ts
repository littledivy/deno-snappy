...
if (!(await Deno.stat("Cargo.toml")).isFile) {
  err(`the build script should be executed in the "${proj_name}" root`);
}
...
const wasm = await Deno.readFile(`pkg/${proj_name}_bg.wasm`);
...
const init = await Deno.readTextFile(`pkg/${proj_name}.js`);
